 ...
 *
 * Date: Wed, 12 May 2021 09:45:34 GMT
 */
...
MCSLiteDevice mcs("SksBwfFOO", "b7ce93bbef94107e2d139ba85b7fcdb84ba827a56aa879782816bcd63b94c85e", "192.168.1.132", 3000);

MCSDisplayInteger mcs_t("mcs_t");
MCSDisplayInteger mcs_h("mcs_h");
MCSDisplayInteger mcs_f("mcs_f");
MCSDisplayInteger mcs_r("mcs_r");
MCSDisplayInteger mcs_l("mcs_l");
MCSControllerOnOff mcs_s("mcs_s");
MCSControllerInteger mcs_p("mcs_p");
MCSControllerInteger mcs_g("mcs_g");
MCSControllerInteger mcs_b("mcs_b");
...
void setup()
{
  ...
  mcs.addChannel(mcs_t);
  mcs.addChannel(mcs_h);
  mcs.addChannel(mcs_f);
  mcs.addChannel(mcs_r);
  mcs.addChannel(mcs_l);
  mcs.addChannel(mcs_s);
  mcs.addChannel(mcs_p);
  mcs.addChannel(mcs_g);
  mcs.addChannel(mcs_b);
...
void loop()
{
  ...
  if (mcs_p.updated()) {
    analogWrite(4, mcs_p.value());

  }
  if (mcs_g.updated()) {
    analogWrite(5, mcs_g.value());

  }
  if (mcs_b.updated()) {
    analogWrite(3, mcs_b.value());
